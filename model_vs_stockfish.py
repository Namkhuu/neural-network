...
def get_intermediate_reward(prev_board, new_board):
    ...
    last_move = new_board.peek() if new_board.move_stack else None
    if last_move and last_move.to_square in center_squares:
        move_number = new_board.fullmove_number
        center_bonus = 0.5 * max(0.1, 1.0 - (move_number / 40))
        reward += center_bonus
...
try:
    actor_model = load_model("best_actor_model.keras")
    critic_model = load_model("best_ critic_model.keras")
    ...
    critic_hidden = Dense(128, activation='relu')(critic_hidden)
...
gamma = 0.99
num_episodes = 500
best_total_reward = float('-inf')  # or -np.inf

# ===== Start Stockfish Engine =====
engine = chess.engine.SimpleEngine.popen_uci("stockfish/stockfish-windows-x86-64-avx2.exe")
engine.configure({"Skill Level": 2})  # Weakest level
...
for ep in range(num_episodes):
    ...
    while not board.is_game_over():
        if board.turn == chess.WHITE:
            ...
            for move in board.legal_moves:
                ...
                sim_board.push(move)
                score = rollout_value(sim_board, depth=3)
                move_scores.append((move, score))

            move = max(move_scores, key=lambda x: x[1])[0]
            action_uci = move.uci()
            action_index = MOVE_IDX[action_uci]


            if move not in board.legal_moves:
                print("Illegal move attempted, skipping episode.")
                break
            ...
            with tf.GradientTape() as tape:
                ...
                log_prob = tf.math.log(probs[action_index] + 1e-8)
                entropy = -tf.reduce_sum(probs * tf.math.log(probs + 1e-8))
                actor_loss = -log_prob * advantage - 0.01 * entropy  # Entropy bonus
    ...
    if total_reward > best_total_reward:
        ...
        actor_model.save("best_actor_model.keras")
        critic_model.save("best_critic_model.keras")
